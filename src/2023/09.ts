...
type FirstChar<Str> =  Str extends `${infer S}${string}` ? S : never;

type FL = FirstChar<'abc'>

type Tail<Str> = Str extends `${string}${infer E}` ? E : never;

type TailResult = Tail<'abc'>

type LastLetter<Str> = Str extends `${infer A}${infer B}` ? "" extends B ? A : LastLetter<B> : Str;

type LLResult = LastLetter<'abcd'>

type Reverse<Str> = Str extends `${infer A}${infer B}` ? `${"" extends B ? B : Reverse<B> }${A}` :  never;
